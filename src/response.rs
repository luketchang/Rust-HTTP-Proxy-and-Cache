...
pub fn send_response(client_stream: &mut TcpStream, response: &http::Response<Vec<u8>>) {
    let client_ip = client_stream.peer_addr().unwrap().ip().to_string();
    log::info!("{} <- {}", client_ip, format_response_line(&response));
    if let Err(error) = write_to_stream(&response, client_stream) {
